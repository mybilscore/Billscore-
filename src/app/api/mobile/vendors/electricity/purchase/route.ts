  ...
  try {
    ...
    if (!customer) {
      ...
      if (!customer) {
        ...
        CacheService.setCustomer(user.id, customerPhone || user.phone, customer).catch(() => {});
      }
    }
    ...
    try {
      ...
      // ============================================================
      // 10. SUCCESS - Complete transaction
      // ============================================================
      ...
      await Promise.all([
        ...
        CacheService.invalidateCustomer(user.id, customerPhone || user.phone),
      ...
      await prisma.vtuTransaction.update({
        ...
        data: {
          ...
          metadata: {
            ...transaction.metadata,
            error: vendorError.message || "Vendor transaction failed",
            failedAt: new Date().toISOString(),
            wasDebited: false,
          },
        },
      ...
      await prisma.customerTransaction.create({
        data: {
          ...
          meterNumber: meterNumber,
          status: TransactionStatus.FAILED,
          notes: `Failed: ${vendorError.message || "Vendor transaction failed"}`,
          metadata: {
      ...
      log('error', 'Mobile electricity purchase failed', vendorError.message);
