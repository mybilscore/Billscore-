// src/app/api/mobile/vendors/subscription/create/route.ts
...
async function createFailedSubscriptionTransaction(
  userId: string,
  ...
  try {
    ...
    await prisma.customerTransaction.create({
      data: {
        ...
        meterNumber: meterNumber,
        status: TransactionStatus.FAILED,
        notes: `Subscription failed: ${errorMessage}`,
        metadata: {
          failureReason: failureReason,
          serviceType: "electricity",
          failedAt: new Date().toISOString(),
    ...
    console.error("❌ [MOBILE SUBSCRIPTION] Failed to create failed transaction record:", err);
    return null;
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const [cachedUser, cachedCustomer, cachedBalance] = await Promise.all([
      ...
      CacheService.getCustomer(userId, decoded.phone || '').catch(() => null),
    ...
    if (!customer) {
      const phoneToUse = decoded.phone || user.phone || '';
    ...
    log('info', `Mobile subscription created in ${totalTime}ms`);
    ...
    log('error', 'Mobile subscription creation failed', error.message);
    return NextResponse.json({
      success: false,
