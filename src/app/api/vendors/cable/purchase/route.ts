  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      const wt = await tx.walletTransaction.create({
        data: {
          ...
          description: `Refund for failed cable subscription: ${reason}`,
    ...
    await prisma.refundNotification.create({
      data: {
        ...
        type: "COMPLETED",
        channel: "MOBILE_PUSH",
        message: `Your refund of ₦${amount} for cable subscription has been processed.`,
        metadata: {
          refundId: refund.id,
  ...
  try {
    ...
    try {
      ...
      if (result.success) {
        ...
        log('info', `Cable transaction ${transaction.id} completed in ${totalTime}ms`);
      ...
      log('error', 'Cable purchase failed', error.message);
