...
const registrationSchema = z.object({
  ...
  /** Individual's full name — used for the account holder */
  ...
  pin: z
    ...
    .optional(),
  role: z
    .enum(["END_USER", "RETAILER", "AGENT", "ADMIN", "SUPER_ADMIN", "DEVELOPER"])
    .default("END_USER"),
...
export async function POST(request: NextRequest) {
  try {
    ...
    // ─────────────── STEP 2: BRUTE FORCE ───────────────
    const emailBlocked = await registrationBruteForce.isBlocked(
      body.email,
      ip as string
    );
    ...
    if (
      ...
      validated.password !== validated.confirmPassword
    ) {
    ...
    if (existingUser) {
      ...
      else if (existingUser.username === validated.username?.toLowerCase().trim())
        field = "Username";

      await registrationBruteForce.recordFailedAttempt(body.email, ip as string, {
        reason: "user_exists",
        field,
      });
    ...
    const pinToUse = validated.pin || "1234";
    ...
    try {
      ...
      // ✅ Audit BEFORE rollback (user still exists)
      try {
      ...
      const isConfigError = /PALMPAY_CAC_NUMBER|CAC|configuration/i.test(
        error.message
      );
      ...
      return NextResponse.json(
        {
          ...
          error: isConfigError
            ...
            : isIdentityError
            ? "We couldn't verify the business registration. Please contact support."
            : "We couldn't set up your wallet right now. Please try again in a moment. If the problem persists, contact support.",
          code: isConfigError
        ...
        { status: isConfigError ? 500 : isIdentityError ? 400 : 503 }
      );
    }

    // ─────────────── STEP 10: REFERRAL RECORD (after wallet success) ───────────────
    if (referrerId) {
      try {
    ...
    return NextResponse.json(
      {
        ...
        virtualAccount: virtualAccountNo
          ? { accountNumber: virtualAccountNo, isSimulation }
          : null,
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, data: { username, available } });
  } catch (error: any) {
