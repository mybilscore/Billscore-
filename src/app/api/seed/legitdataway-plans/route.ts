// src/app/api/seed/legitdataway-plans/route.ts

import { NextRequest, NextResponse } from "next/server";
import { prisma } from "~/lib/db";
import { PlanImportService } from "~/lib/services/plan-import.service";

// All LegitDataway plans - Same structure as BilalSada
const LEGITDATAWAY_PLANS = [
...
export async function POST(request: NextRequest) {
  try {
    ...
    const vendorCode = body.vendorCode || 'LEGITDATAWAY';
    const importedBy = body.importedBy || 'system';

    console.log(`🌱 [Seed] Importing LegitDataway plans for ${vendorCode}...`);
    console.log(`📊 [Seed] Total plans to import: ${LEGITDATAWAY_PLANS.length}`);
    ...
    if (!vendor) {
      return NextResponse.json({
        ...
        steps: [
          '1. POST /api/seed/legitdataway - to create the vendor',
          '2. POST /api/seed/legitdataway-plans - to import plans'
        ]
      }, { status: 404 });
    }
    ...
    return NextResponse.json({
      success: true,
      message: `LegitDataway plans imported successfully (${result.created} created, ${result.updated} updated)`,
      data: {
        vendor: {
          id: vendor.id,
          code: vendor.code,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const vendorCode = searchParams.get('vendorCode') || 'LEGITDATAWAY';
